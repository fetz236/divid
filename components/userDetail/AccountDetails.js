...
export default function AccountDetails({navigation, ...props}) {
    ...
    useEffect(async() => {
        const getCategories = () => {
            ...
            for(let i=0; i<categories.length; i++){
                for(let j=0; j<props.route.params.user_data.categories.length; j++){
                    if (categories[i].__id__ == props.route.params.user_data.categories[j]){
    ...
    return (
        <View style={account_details_style.main_container}>
            ...
            <View>
                <AccountDetailsBody/>
                <AccountFirstName f_name={f_name} setF_name={setF_name} fView={fView} setfView={setfView}/>
                <AccountLastName l_name={l_name} setL_name={setL_name} lView={lView} setlView={setlView}/>
                <AccountMobile mobile={mobile} mobileCountry={mobileCountry}
                 setMobile={setMobile} mView={mView} setmView={setmView} setMobileCountry={setMobileCountry}
                 setMobileCountryCallingCode={setMobileCountryCallingCode}/>
                <>
                    <AccountInterests category_icons={category_icons} onSelectedItemsChanged={onSelectedItemsChanged}/>
                </>
                <View>
...
const SubmitInfo = (props) => (
    <View>
        <TouchableOpacity style={account_details_style.submit_button}
